...
const BillEntryView = ({ companyId }) => {
  ...
  useEffect(() => {
    if (companyId && id) {
      Promise.all([
        purchaseAPI.getBills(companyId),
  ...
  const handleSaveOrder = async (sendEmail = false) => {
    ...
    try {
      const payload = {
        ...
        date: formData.date,
        dueDate: formData.deliveryDate || null,
      ...
      let res;
      if (id) {
        res = await purchaseAPI.updateBill(id, payload);
      } else {
        res = await purchaseAPI.createBill(payload);
      }
          ...
          <div className="flex-1 px-8 py-6 max-w-[1200px]">
             ...
             <div className="mt-8 pb-4 border-t border-slate-100 pt-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
                ...
                <div className="mt-6 pt-4 text-[12px] text-slate-500 border-t border-slate-50">
                   Additional Fields: Start adding custom fields for your purchase orders by going to Settings ➔ Purchases ➔ Bills.
                </div>
             </div>
          </div>
       </div>
