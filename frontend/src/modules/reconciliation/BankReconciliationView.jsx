            ...
            <div className="bg-gradient-to-br from-primary-600 to-primary-700 p-10 rounded-4xl text-white shadow-xl relative overflow-hidden group">
               <div className="relative z-10 flex justify-between items-center">
                  ...
                  <button 
                    ...
                    className="bg-white text-primary-600 px-8 py-4 rounded-2xl font-bold shadow-lg hover:scale-105 transition-all flex items-center gap-2"
                  >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white rounded-4xl border border-gray-100 shadow-card overflow-hidden h-fit">
                    <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-surface-50">
                        <h4 className="font-bold text-ink-900 flex items-center gap-2"><FileText size={18}/> Unreconciled Bank Entries</h4>
                        <span className="bg-amber-100 text-amber-700 text-[10px] font-bold px-2.5 py-1 rounded-full">{unmatched.length} Pending</span>
                    </div>
                                ...
                                <div key={entry.id} onClick={() => findMatches(entry)} className={`p-6 cursor-pointer hover:bg-primary-50 transition-all group ${selectedEntry?.id === entry.id ? 'bg-primary-50 border-l-4 border-primary-500' : ''}`}>
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <p className="text-[10px] font-bold text-ink-400 uppercase">{new Date(entry.date).toLocaleDateString()}</p>
                                        </div>
                                        <div className="text-right">
                                            <p className={`font-bold ${entry.type === 'Credit' ? 'text-emerald-600' : 'text-red-500'}`}>
                ...
                {selectedEntry ? (
                    <div className="bg-white rounded-4xl border border-gray-100 shadow-card overflow-hidden animate-fade-scale">
                        <div className="p-8 border-b border-gray-100 bg-ink-900 text-white flex justify-between items-center">
                            <h4 className="font-bold">Potential Voucher Matches</h4>
                                ...
                                matchingVouchers.map(v => (
                                    <div key={v.id} className="p-6 bg-surface-50 rounded-3xl border border-gray-100 group relative">
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                <p className="text-xs font-bold text-ink-400 uppercase tracking-widest">{v.voucherType} #{v.voucherNumber}</p>
                                                <p className="font-bold text-ink-800">{v.narration || 'No Narration'}</p>
                                            </div>
                                            <p className="font-bold">₹{parseFloat(selectedEntry.amount).toLocaleString()}</p>
                                        </div>
                                        <button 
                                          onClick={() => reconcile(v.id)}
                                          className="w-full py-3 bg-ink-900 text-white rounded-2xl font-bold text-xs shadow-lg hover:bg-primary-600 transition-all flex items-center justify-center gap-2"
                                        >
                    ...
                    <div className="bg-surface-50 border-2 border-dashed border-gray-100 rounded-4xl flex flex-col items-center justify-center p-20 text-center space-y-4">
                        ...
                        <h4 className="text-xl font-bold text-ink-200">Select an entry to begin matching</h4>
                    </div>
                )}
            </div>
        </div>
